...
#Region "Menu Event"
    ...
    Private Sub _SBO_Application_FormDataEvent(ByRef BusinessObjectInfo As SAPbouiCOM.BusinessObjectInfo, ByRef BubbleEvent As Boolean) Handles _SBO_Application.FormDataEvent
        Select Case BusinessObjectInfo.FormTypeEx
            Case frm_CEC
                Dim objInvoice As clsCEC
                objInvoice = New clsCEC
            ...
            Case "139"
                ...
                If BusinessObjectInfo.EventType = SAPbouiCOM.BoEventTypes.et_FORM_DATA_ADD And BusinessObjectInfo.BeforeAction = False And BusinessObjectInfo.ActionSuccess = True Then
                    ' oJe = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oDeliveryNotes)
                    oJe = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oOrders)
                    If oJe.Browser.GetByKeys(BusinessObjectInfo.ObjectKey) Then
                        If oApplication.Utilities.CreateCustomerEquipmentCard_Order(oJe.DocEntry) = True Then
                            oApplication.Utilities.Message("Customer Equipment Cards are created ", SAPbouiCOM.BoStatusBarMessageType.smt_Success)
                        End If
                    End If
                End If
                    ...
                    Case "139"
                        If pVal.EventType = SAPbouiCOM.BoEventTypes.et_FORM_LOAD Then
                            ...
                            If oForm.TypeEx = "139" Then
                                oApplication.Utilities.AddControls(oForm, "stEnd", "86", SAPbouiCOM.BoFormItemTypes.it_STATIC, "DOWN", 0, 0, , "End Customer")
                                oApplication.Utilities.AddControls(oForm, "edEnd", "46", SAPbouiCOM.BoFormItemTypes.it_EDIT, "DOWN", 0, 0, )
                                Dim oEdit As SAPbouiCOM.EditText
                                Dim oItem As SAPbouiCOM.Item
                                oEdit = oForm.Items.Item("edEnd").Specific
                                oEdit.DataBind.SetBound(True, "ORDR", "U_EndCustomer")
                                oItem = oForm.Items.Item("stEnd")
                                oItem.LinkTo = "edEnd"

                            End If
                        End If
                    Case frm_CECAttribute
                        If Not _Collection.ContainsKey(FormUID) Then
                            oItemObject = New clsAttribute
                            oItemObject.FrmUID = FormUID
                            _Collection.Add(FormUID, oItemObject)
                        End If
                    Case frm_CEC
                        If Not _Collection.ContainsKey(FormUID) Then
                            oItemObject = New clsCEC
                            oItemObject.FrmUID = FormUID
                            _Collection.Add(FormUID, oItemObject)
                        End If
                        'Case "65052"
