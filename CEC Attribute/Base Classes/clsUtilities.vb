...
Imports CEC.WebReference.UBSWebservice
Imports CEC.WebReference
...
#Region "BisB Integration"
    Public Function SqlConnectionString() As String
        ...
        If oRec.RecordCount > 0 Then
            ...
            ConnectionString = "data source=" & sername & ";Integrated Security=SSPI;database=" & dbname & ";User id=" & uid & "; password=" & pwd
        End If
                ...
                Select Case BusinessObjectInfo.FormTypeEx
                    ...
                    Case "392" 'Journal Entry
                        ...
                        If oJE.Browser.GetByKeys(BusinessObjectInfo.ObjectKey) Then
                            ExportJournalEntries_Testing(oJE.Number)
                        End If
    ...
    Public Sub ExportJournalEntries_Testing(ByVal aPath As String)
        Dim oRecItem, oRecItemCode, oTemp, oMainRec As SAPbobsCOM.Recordset
        Dim strSQL, strFilename As String
        Dim sValue As String
        Dim sPath, strLogDirectory, strPath, strMessage, strSelectedFolderPath, strExportFilePaty As String
    ...
    Private Function SendXMLtoIFX(ByVal aString As String) As String
        ...
        'Dim MyService1 As New CEC.WebReference.UBSWebservice
...
#End Region
    ...
    Public Function CreateCustomerEquipmentCard(ByVal aDocEntry As Integer) As Boolean
        Dim oCEC As SAPbobsCOM.CustomerEquipmentCards
        ...
        If oDoc.GetByKey(aDocEntry) Then
            If oDoc.DocType = SAPbobsCOM.BoDocumentTypes.dDocument_Items Then
                If oDoc.UserFields.Fields.Item("U_EndCustomer").Value <> "" Then
                    strCode = oDoc.UserFields.Fields.Item("U_EndCustomer").Value
                Else
                    strCode = oDoc.CardCode
                End If
                For intRow As Integer = 0 To oDoc.Lines.Count - 1
                    oDoc.Lines.SetCurrentLine(intRow)
                    If oItem.GetByKey(oDoc.Lines.ItemCode) Then
                        If oItem.UserFields.Fields.Item("U_CEC").Value = "Y" Then
                            If oItem.ManageSerialNumbers = SAPbobsCOM.BoYesNoEnum.tYES Then
                                For intLoop As Integer = 0 To oDoc.Lines.SerialNumbers.Count - 1
                                    oDoc.Lines.SerialNumbers.SetCurrentLine(intLoop)
                                    oCEC = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oCustomerEquipmentCards)
                                    oCEC.CustomerCode = strCode
                                    oCEC.ItemCode = oDoc.Lines.ItemCode
                                    oCEC.ManufacturerSerialNum = oDoc.Lines.SerialNumbers.ManufacturerSerialNumber
                                    oCEC.InternalSerialNum = oDoc.Lines.SerialNumbers.InternalSerialNumber
                                    oCEC.UserFields.Fields.Item("U_QTY").Value = 1
                                    oCEC.DeliveryCode = oDoc.DocEntry
                                    If oCEC.Add <> 0 Then
                            ...
                            Else
                                ...
                                oCEC.CustomerCode = strCode
                                oCEC.ItemCode = oDoc.Lines.ItemCode
                                oCEC.ManufacturerSerialNum = oDoc.DocNum
                                oCEC.InternalSerialNum = oDoc.DocNum
                                oCEC.DeliveryCode = oDoc.DocEntry
                                oCEC.UserFields.Fields.Item("U_QTY").Value = oDoc.Lines.Quantity
                                If oCEC.Add <> 0 Then
                                    oApplication.Utilities.Message(oApplication.Company.GetLastErrorDescription, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                                    Return False
                                End If
    ...
    Public Function CreateCustomerEquipmentCard_Order(ByVal aDocEntry As Integer) As Boolean
        Dim oCEC As SAPbobsCOM.CustomerEquipmentCards
        Dim strCode As String
        Dim oDoc As SAPbobsCOM.Documents
        Dim oItem As SAPbobsCOM.Items
            ...
            If oDoc.DocType = SAPbobsCOM.BoDocumentTypes.dDocument_Items Then
                ...
                For intRow As Integer = 0 To oDoc.Lines.Count - 1
                    ...
                    If oItem.GetByKey(oDoc.Lines.ItemCode) Then
                        If oItem.UserFields.Fields.Item("U_CEC").Value = "Y" Then
                            If oItem.ManageSerialNumbers = SAPbobsCOM.BoYesNoEnum.tYES Then
                                Dim blnSerialExists As Boolean = False
                                For intLoop As Integer = 0 To oDoc.Lines.SerialNumbers.Count - 1
                                    If oDoc.Lines.SerialNumbers.InternalSerialNumber <> "" Then
                                        blnSerialExists = True
                                        oDoc.Lines.SerialNumbers.SetCurrentLine(intLoop)
                                        oCEC = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oCustomerEquipmentCards)
                                        oCEC.CustomerCode = strCode
                                        oCEC.ItemCode = oDoc.Lines.ItemCode
                                        oCEC.UserFields.Fields.Item("U_Z_MacCode").Value = oDoc.Lines.UserFields.Fields.Item("U_Z_MacCode").Value
                                        oCEC.ManufacturerSerialNum = oDoc.Lines.SerialNumbers.ManufacturerSerialNumber
                                        oCEC.InternalSerialNum = oDoc.Lines.SerialNumbers.InternalSerialNumber
                                        oCEC.UserFields.Fields.Item("U_QTY").Value = 1
                                        oCEC.DeliveryCode = oDoc.DocEntry
                                        oCEC.DirectCustomerCode = oDoc.CardCode
                                        oCEC.DirectCustomerName = oDoc.CardName
                                        If blnSerialExists = True Then
                                            If oCEC.Add <> 0 Then
                                                oApplication.Utilities.Message(oApplication.Company.GetLastErrorDescription, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                                            Else
                                                blnSerialExists = True
                                ...
                                If blnSerialExists = False Then
                                    oCEC = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oCustomerEquipmentCards)
                                    oCEC.CustomerCode = strCode
                                    oCEC.ItemCode = oDoc.Lines.ItemCode
                                    oCEC.UserFields.Fields.Item("U_Z_MacCode").Value = oDoc.Lines.UserFields.Fields.Item("U_Z_MacCode").Value
                                    oCEC.ManufacturerSerialNum = "#" & oDoc.DocNum.ToString
                                    oCEC.InternalSerialNum = "#" & oDoc.DocNum.ToString
                                    'oCec.d.DeliveryCode = oDoc.DocEntry
                                    oCEC.UserFields.Fields.Item("U_QTY").Value = oDoc.Lines.Quantity
                                    oCEC.DirectCustomerCode = oDoc.CardCode
                                    oCEC.DirectCustomerName = oDoc.CardName
                                    If oCEC.Add <> 0 Then
                            ...
                            Else
                                ...
                                oCEC.ItemCode = oDoc.Lines.ItemCode
                                oCEC.UserFields.Fields.Item("U_Z_MacCode").Value = oDoc.Lines.UserFields.Fields.Item("U_Z_MacCode").Value
                                'oCEC.ManufacturerSerialNum = "#" & oDoc.Lines.LineNum.ToString & oDoc.DocNum.ToString
                                ''  oCEC.InternalSerialNum = oDoc.DocNum
                                'oCEC.InternalSerialNum = "#" & oDoc.Lines.LineNum.ToString & oDoc.DocNum.ToString
                                oCEC.ManufacturerSerialNum = "#" & oDoc.DocNum.ToString
                                oCEC.InternalSerialNum = "#" & oDoc.DocNum.ToString
                                oCEC.UserFields.Fields.Item("U_QTY").Value = oDoc.Lines.Quantity
                                oCEC.DirectCustomerCode = oDoc.CardCode
